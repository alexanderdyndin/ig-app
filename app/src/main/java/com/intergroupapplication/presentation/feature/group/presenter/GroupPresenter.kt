...
class GroupPresenter @Inject constructor(
    ...
    fun attachFromGallery(groupId: String? = null) {
        stopImageUploading()
        uploadingImageDisposable = imageUploadingDelegate.uploadFromGallery(
            viewState,
            errorHandler,
            groupId,
            photoGateway::uploadAvatarGroup
        )
    }

    fun attachFromCamera(groupId: String?) {
        stopImageUploading()
        uploadingImageDisposable = imageUploadingDelegate.uploadFromCamera(
            viewState,
            errorHandler,
            groupId,
            photoGateway::uploadAvatarGroup
