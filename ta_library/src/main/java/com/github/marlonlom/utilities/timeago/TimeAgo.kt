...
private constructor() {
    ...
    companion object {
        ...
        const val TRUE_TIME_NOT_UPLOADED = -100000000L

        @JvmStatic
        @JvmOverloads
        fun using(time: Long, resources: TimeAgoMessages = TimeAgoMessages.Builder().defaultLocale().build(),
                  currentTime: Long): String {
            val dim = getTimeDistanceInMinutes(time, currentTime)
         ...
         */
        private fun getTimeDistanceInMinutes(time: Long, currentTime: Long): Long =
                doOrIfConditional({ currentTime != TRUE_TIME_NOT_UPLOADED }, {
                    val timeDistance = currentTime - time
                    return Math.round((timeDistance / 1000 / 60).toFloat()).toLong()
                }, {
                    val dateFormat = SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss.SSS'Z'", Locale.getDefault())
                            .apply { timeZone = TimeZone.getTimeZone("GMT") }
                    val nowDate = dateFormat.parse(Instant.now().toString())
                    val timeDistance = nowDate.time - time
                    return Math.round((timeDistance / 1000 / 60).toFloat()).toLong()
                })
    }
}
